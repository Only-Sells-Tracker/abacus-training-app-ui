...
export function TrainingGame({ tournament, onBack }: TrainingGameProps) {
  ...
  const generateNumbers = () => {
    const min = Math.pow(10, tournament.digitCount - 1);
    const max = Math.pow(10, tournament.digitCount) - 1;
    const newNumbers: NumberItem[] = [];

    for (let i = 0; i < tournament.numberCount; i++) {
      const value = Math.floor(Math.random() * (max - min + 1)) + min;
      const operation = tournament.operations[Math.floor(Math.random() * tournament.operations.length)];
  ...
  }, [gameState, currentIndex, numbers.length, tournament.delay]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-slate-900 to-black">
      {/* Header */}
      <div className="bg-white/5 backdrop-blur-xl border-b border-white/10 px-4 py-4 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 bg-gradient-to-br ${tournament.color} rounded-xl flex items-center justify-center text-xl`}>
              {tournament.icon}
            </div>
            <div>
              <h2 className="text-lg text-white">{tournament.planet}</h2>
              <p className="text-xs text-gray-400">{tournament.digitCount} digits • {tournament.numberCount} numbers</p>
            </div>
          </div>
        ...
        <div className="w-full max-w-2xl">
          <AnimatePresence mode="wait">
            ...
            {gameState === 'ready' && (
              ...
              >
                <div className={`w-32 h-32 bg-gradient-to-br ${tournament.color} rounded-full flex items-center justify-center text-6xl mx-auto mb-8 shadow-2xl`}>
                  {tournament.icon}
                </div>
                <h2 className="text-3xl mb-4 text-white">Ready to Train?</h2>
                <p className="text-gray-400 mb-8 max-w-md mx-auto">
                  {tournament.numberCount} numbers will appear one by one. Calculate the result in your mind!
                </p>
                <motion.button
              ...
              >
                ...
                <div className="w-full bg-white/10 rounded-full h-2 mb-12 overflow-hidden">
                  <motion.div
                    ...
                    className={`h-full bg-gradient-to-r ${tournament.color}`}
